...
def with_retries(
    attempts: int, expected_ex: Exception, callback: Callable[[], Any]
) -> Any:
    """
    This is a generic function that retries a callback a given number of times, when the callback throws the expected exception.
    It sleeps for 5 seconds between retries.
    """
...
def _fetch_civic_table():
    ...
    while page_url is not None:
        ...
        for variant in records:
            row = {}
            if (
                variant["coordinates"]["start"] is None
                or variant["coordinates"]["chromosome"] is None
            ):
                continue
